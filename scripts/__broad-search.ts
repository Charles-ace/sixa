...
async function main() {
  const init = await rpc("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "sixa-broad-search", version: "1.0.0" } });
  if (init.status !== 200) { console.log(init.raw); return; }
  await rpc("notifications/initialized", {});
  for (const q of ["sepolia", "gas refuel", "testnet"]) {
    const r = await rpc("tools/call", { name: "search_workflows", arguments: { query: q } });
    console.log(`\n=== query="${q}" HTTP ${r.status} ===`);
    console.log(innerText(r.raw));
  }
}
