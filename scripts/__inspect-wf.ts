...
async function main() {
  const init = await rpc("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "sixa-node-enable", version: "1.0.0" } });
  console.log(`init HTTP ${init.status}`);
  if (init.status !== 200) return;
  await rpc("notifications/initialized", {});

  console.log("\n=== CURRENT WORKFLOW STATE ===");
  const g = await rpc("tools/call", { name: "get_workflow", arguments: { workflowId: "9ddjdvhrqouokxzmf42xn" } });
  const wf = JSON.parse(innerText(g.raw));
  console.log(`workflow.enabled: ${wf.enabled}`);
  for (const n of wf.nodes ?? []) {
    console.log(`node ${n.id} type=${n.type} nodeData.enabled=${n.data?.enabled} label=${n.data?.label ?? ""} actionType=${n.data?.config?.actionType ?? n.data?.config?.triggerType ?? ""} network=${n.data?.config?.network ?? "-"}`);
  }
}
