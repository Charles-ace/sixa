...
async function main() {
  const init = await rpc("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "sixa-schema", version: "1.0.0" } });
  console.log(`init HTTP ${init.status}`);
  await rpc("notifications/initialized", {});

  console.log("\n=== list_integrations ===");
  const ints = await rpc("tools/call", { name: "list_integrations", arguments: {} });
  console.log(`HTTP ${ints.status}`);
  console.log(innerText(ints.raw));

  console.log("\n=== list_action_schemas (category=web3) ===");
  const schemas = await rpc("tools/call", { name: "list_action_schemas", arguments: { category: "web3" } });
  console.log(`HTTP ${schemas.status}`);
  const text = innerText(schemas.raw);
