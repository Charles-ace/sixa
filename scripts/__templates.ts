...
async function main() {
  const init = await rpc("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "sixa-templates", version: "1.0.0" } });
  if (init.status !== 200) return;
  await rpc("notifications/initialized", {});
  for (const q of ["pause", "settle", "transfer", "write"]) {
    const r = await rpc("tools/call", { name: "search_templates", arguments: { query: q, limit: 10 } });
    console.log(`\n=== search_templates query="${q}" HTTP ${r.status} ===`);
    console.log(innerText(r.raw).slice(0, 2000));
  }
}
