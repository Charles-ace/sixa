...
async function main() {
  const init = await rpc("initialize", { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "sixa-tpl-top5", version: "1.0.0" } });
  console.log(`init HTTP ${init.status}`);
  if (init.status !== 200) return;
  await rpc("notifications/initialized", {});
  for (const limit of [5, 10]) {
    const r = await rpc("tools/call", { name: "search_templates", arguments: { query: "Verify the anchor commitment on Base for the demo run", limit } });
    console.log(`\n=== search_templates limit=${limit} HTTP ${r.status} ===`);
