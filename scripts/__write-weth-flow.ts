...
async function main() {
  ...
  console.log("--- STEP 0: initialize ---"); console.log(`HTTP ${init.status}`); console.log(init.raw);
  if (init.status !== 200) return;
  await rpc("notifications/initialized", {});

  const slug = "test-sepolia-weth-deposit-issue2";
  console.log(`\n========== STEP 1: callWorkflow (slug="${slug}", inputs={}) ==========`);
  ...
  if (callInner && "x402Version" in callInner) { console.log("\nRESULT: x402 payment quote returned — payment required, no execution."); return; }
  if (call.status === 402) { console.log("\nRESULT: HTTP 402 Payment Required — no executionId, nothing to poll."); return; }
  const executionId = findString(callInner, ["executionId", "id"]);
  if (!executionId) { console.log("\nRESULT: no executionId returned. Cannot poll. No transactionHash."); return; }

  console.log(`\n========== STEP 2: waitForExecution (get_execution every 3s, max 40 polls) ==========`);
  ...
  else console.log("\nMEANING: terminal state without a transactionHash field — read-only execution or no on-chain tx. NOT proven.");
}
