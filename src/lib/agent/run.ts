...
async function handleWorkflowRoute(message: string, history: unknown[], walletAddress: string | undefined) {
  ...
  if (/(resume|enable|start|approve|make autonomous|run autonomously)\s+(.*)/i.test(message)) {
    ...
    const match = message.toLowerCase();
    const workflows = await provider.listWorkflows();
    const target = workflows.find((w) => match.toLowerCase().includes(w.name.toLowerCase()));
    if (target) {
...
export async function runAgentRequest(input: AgentRequest): Promise<AgentResponse> {
  const { message, history = [], walletAddress, chainId, accountAddress, accountEmail } = input;

  if (!message || typeof message !== 'string') {
    throw new AgentApiError('Message is required', 400);
  }
  ...
  if (isWorkflowRequest(message) || /deploy (it|this|the)/i.test(message)) {
    try {
      ...
      return { ...result, workflowIntent: true, executable: false };
    } catch (error) {
      console.error('Workflow route error:', error);
      throw new AgentApiError(
        error instanceof Error ? error.message : 'Workflow handling failed',
        500,
        'workflow_failed',
        { workflowIntent: true }
      );
  ...
  if (intent.type === 'portfolio' || intent.type === 'balance' || intent.type === 'history') {
    ...
    try {
      ...
      return { ...result, executable: false, simulation: null };
    } catch (error) {
      return {
        content: `I couldn't pull ${intent.type} data from the chain right now. Add a wallet or try again — ${error instanceof Error ? error.message : 'unknown error'}`,
        intent,
        executable: false,
        simulation: null,
      };
